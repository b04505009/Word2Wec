...
int VocabCompare(const void* a, const void* b) {
    // what I do: exchange a and b
    return ((struct vocab_word*)a)->cn - ((struct vocab_word*)b)->cn;
}
...
void SortVocab() {
    ...
    for (a = 0; a < size; a++) {
        ...
        if ((vocab[a].cn < min_count) && (a != 0)) {
            ...
            while (vocab_hash[hash] != -1)
                hash = (hash + 1) % vocab_hash_size;
            // what I do: a minus startpoint
            vocab_hash[hash] = a - startpoint;
            train_words += vocab[a].cn;
        }
    }
    vocab_size--;
    // printf("vocab_size: %lli\n", vocab_size);
    // printf("startpoint: %lli\n", startpoint);
    // fflush(stdout);
    // for (int i = startpoint; i < size - 1; i++) {
    //    printf("vocab[%i].cn: %lli\n", i, vocab[i].cn);
    //    fflush(stdout);
    //}
    ...
    struct vocab_word* vocab1 =
        (struct vocab_word*)calloc((vocab_size), sizeof(struct vocab_word));
    memcpy(vocab1, &vocab[startpoint],
...
void CreateBinaryTree() {
    ...
    // what I do: create count alias
    long long* count_alias =
        (long long*)calloc(vocab_size * 2 + 1, sizeof(long long));

    for (a = 0; a < vocab_size; a++) {
        count[a] = vocab[a].cn;
        count_alias[a] = vocab[a].cn;
        // printf("count[%lli]: %lli\n", a, count[a]);
    }
    fflush(stdout);

    // what I do: save max freq
    int max_freq = count[vocab_size - 1];
    max_freq++;
    for (a = 0; a < vocab_size; a++) {
        // what I do: reverse freq
        count_alias[a] = max_freq - vocab[a].cn;
        // printf("count_alias[%lli]: %lli\n", a, count_alias[a]);
    }
    fflush(stdout);

    for (a = vocab_size; a < vocab_size * 2; a++) {
        // what I do: change default count from 1e15 to 0
        count[a] = 1e15;
        count_alias[a] = 1e15;
    }
    pos1 = vocab_size - 1;
    ...
    // what I do: change logic bigger to smaller
    for (a = 0; a < vocab_size - 1; a++) {
        // First, find two smallest nodes 'min1, min2'
        if (pos1 >= 0) {
            if (count_alias[pos1] < count_alias[pos2]) {
                min1i = pos1;
                pos1--;
            } else {
                min1i = pos2;
        ...
        count_alias[vocab_size + a] = count_alias[min1i] + count_alias[min2i];
...
void* TrainModelThread(void* id) {
    ...
    if (load != 1) {
                ...
                if (cw) {
                    ...
                    if (hs) {
                        ...
                        for (d = 0; d < vocab[word].codelen; d++) {
                            f = 0;
                            // l2 - The index of our output word within the
        ...
        if (negative > 0) {
            FILE* fsyn1 = fopen("syn1neg", "rb");
            fread(syn1neg, sizeof(float), sizeof(syn1neg), fsyn1);
            fclose(fsyn1);
        }
        local_iter = 0;
...
for (d = 0; d < negative + 1; d++) {
    if (d == 0) {
        ...
        fprintf(pFile2, "%s,%d,%lli,%f\n", vocab[word].word,
                vocab[word].codelen, vocab[word].cn, my_p2);
